...
eval $(aws ecr get-login --no-include-email --region $AWS_DEFAULT_REGION | sed 's|https://||') #needs AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY envvars
...
SERVICE_ARN="arn:aws:ecs:$AWS_DEFAULT_REGION:$AWS_ACCOUNT_NUMBER:service/$AWS_ECS_CLUSTER_NAME/$ECS_SERVICE"
...
if [ "$(aws ecs list-tasks --cluster $AWS_ECS_CLUSTER_NAME --service-name $ECS_SERVICE --region $AWS_DEFAULT_REGION | jq '.taskArns' | jq 'length')" -gt "0" ]; then
    TEMP_ARN=$(aws ecs list-tasks --service-name $ECS_SERVICE --cluster $AWS_ECS_CLUSTER_NAME --region $AWS_DEFAULT_REGION | jq '.taskArns[0]') # Get current running task ARN
    TASK_ARN="${TEMP_ARN%\"}" # strip double quotes
    TASK_ARN="${TASK_ARN#\"}" # strip double quotes
    aws ecs stop-task --task $TASK_ARN --cluster $AWS_ECS_CLUSTER_NAME --region $AWS_DEFAULT_REGION > /dev/null # Stop current task to force start of new task revision with new image
fi